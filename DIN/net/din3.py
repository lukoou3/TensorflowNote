...
def get_seq_data(data, negsample=0, maxlen=60):
    ...
    for reviewerID, hist in tqdm(data.groupby('user_id')):
        ...
        if len(pos_list) > maxlen:
            pos_list = pos_list[-maxlen:]
    ...
    train_df = pd.DataFrame(train_set, columns=['user_id', 'hist_movie_id', 'movie_id', 'label', 'hist_len']).assign(train=1)
    test_df = pd.DataFrame(test_set, columns=['user_id', 'hist_movie_id', 'movie_id', 'label', 'hist_len']).assign(train=0)

    return pd.concat([train_df, test_df])

def get_data(maxlen=60):
    df_user = pd.read_csv("../../data/ml-1m/users.dat",
                          sep="::", header=None, engine="python",
                          names=["user_id", "gender", "age", "occupation", "zip_code"]).astype({'user_id': 'int32'})
    df_movie = pd.read_csv("../../data/ml-1m/movies.dat",
                           sep="::", header=None, engine="python",
                           names=["movie_id", "title", "genres"]).astype({'movie_id': 'int32'})
    df_rating = pd.read_csv("../../data/ml-1m/ratings.dat",
                            sep="::", header=None, engine="python",
                            names=["user_id", "movie_id", "rating", "ts"]).astype({'user_id': 'int32', 'movie_id': 'int32'})
    ...
    data_df = get_seq_data(df_rating, negsample=1, maxlen=maxlen)
    data_df = pd.merge(pd.merge(data_df, df_user), df_movie)[['user_id', 'gender', 'age', 'hist_movie_id', 'hist_len', 'movie_id', 'genres', 'label', 'train']]

    cate_cols = ["gender", "age", "genres"]
    cate_map = {}
    size = 0
    for col in cate_cols:
        cate_map[col] = {v: i for i, v in enumerate(data_df[col].unique())}
        size += len(cate_map[col])
    for col in cate_cols:
        data_df[col] = data_df[col].map(cate_map[col])

    cate_size = sum([len(map) for col, map in cate_map.items()])
    # print(cate_map)
    print(cate_size)

    return data_df.query("train == 1").iloc[:, :-1], data_df.query("train == 0").iloc[:, :-1], data_df

if __name__ == "__main__":
    maxlen=60
    train_df, test_df, data_df = get_data(maxlen=maxlen)

    X_train = {"user_id": train_df["user_id"], \
               "gender": train_df["gender"], \
               "age": train_df["age"], \
               "hist_movie_id": np.array([x for x in train_df["hist_movie_id"]]), \
               "hist_len": train_df["hist_len"], \
               "movie_id": train_df["movie_id"], \
               "movie_type_id": train_df["genres"]
               }
    y_train = train_df["label"]

    feature_columns = [SparseFeat('user_id', max(data_df["user_id"]) + 1, embedding_dim=8),
                       SparseFeat('gender', max(data_df["gender"]) + 1, embedding_dim=8),
                       SparseFeat('age', max(data_df["age"]) + 1, embedding_dim=8),
                       SparseFeat('movie_id', max(data_df["movie_id"]) + 1, embedding_dim=8),
                       SparseFeat('movie_type_id', max(data_df["genres"]) + 1, embedding_dim=8),
                       DenseFeat('hist_len', 1)]

    feature_columns += [
        VarLenSparseFeat('hist_movie_id', vocabulary_size=max(data_df["movie_id"]) + 1, embedding_dim=8,
                         maxlen=maxlen)]
    ...
    history.compile('adam', 'binary_crossentropy', metrics=["binary_crossentropy", tf.keras.metrics.AUC(name='auc')])

    history.fit(X_train, y_train, batch_size=64, epochs=5, validation_split=0.1, )
