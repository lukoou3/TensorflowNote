...
class word2vec():
    ...
    def build_graph(self):
        ...
        with self.graph.as_default():
            ...
            ##train_inputs  [ 3 5 2 18 ]我   今天     老师  家
            ##train_labels  13300    去
            self.train_inputs = tf.placeholder(tf.int32, shape=[self.batch_size])
            self.train_labels = tf.placeholder(tf.int32, shape=[self.batch_size, 1])
            ## 初始embeding权重（embedding是需要学习得到的）
            #vocab_size（词典大小） * embedding_size（自己提前设定的）
...
if __name__=='__main__':
    ...
    # step 1 读取停用词
    stop_words = []
    with open(r'data/stopwords.txt',encoding="utf-8") as f:
        line = f.readline()
        while line:
    ...
    names = ['易学习', '达康书记', '陈清泉', '肖钢玉', '祁同伟', '高小琴',
             '沙瑞金', '李达康', '陆亦可', '高育良', '吴惠芬', '赵瑞龙 ', '王文革', '赵东来', '郑西坡',
             '程度', '季昌明', '蔡成功', '钟小艾', '田国福', '刘新建', '梁璐', '陈岩石', '王馥真', '丁义珍'
        , '陈海', '赵德汉', '林华华', '周正', '侯亮平','山水集团']
    raw_word_list = []
    sentence_list = []
    # 循环将人名引入词典
    for name in names:
        # jieba.suggest_freq是将一个一个词加入，调用jieba分词时，不会把人名分错
        jieba.suggest_freq(name, True)

    with open(r'data/人民的名义.txt',encoding='utf-8') as f:
        line = f.readline()
        while line:
            while '\n' in line:
                line = line.replace('\n','')
    ...
    w2v = word2vec(vocab_list=word_list,    # 词典集
                   ...
                   logdir='/log_dir')       # tensorboard记录地址
    test_word = ['侯亮平','李达康']
